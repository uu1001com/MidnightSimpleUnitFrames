...
-- =============================================================
-- Coalesced scheduling helpers (perf)
--
-- Goal:
--   Avoid repeated immediate :_ScheduleNext() calls during event storms
--   (Register/SetEnabled/Kick). We coalesce to "next frame".
--
-- Design:
--   * Static callbacks (no per-call closure allocations)
--   * One pending schedule per frame
-- =============================================================
if type(UM._schedulePending) ~= "boolean" then UM._schedulePending = nil end
if type(UM._scheduleWakeFn) ~= "function" then
    UM._scheduleWakeFn = function()
        UM._schedulePending = nil
        UM:_ScheduleNext()
    end
end

-- Kick-clear callback (static) to avoid per-call closure allocations.
if type(UM._kickClearFn) ~= "function" then
    UM._kickClearFn = function()
        UM._kickPending = nil
    end
end

function UM:_RequestScheduleNext()
    if self._schedulePending then
        return
    end
    self._schedulePending = true
    if C_Timer and C_Timer.After then
        C_Timer.After(0, self._scheduleWakeFn)
        return
    end
    -- Fallback: run immediately.
    self._schedulePending = nil
    self:_ScheduleNext()
end
...
function UM:Register(name, fn, interval, priority)
    ...
    self:_Activate(name)
    -- Coalesce scheduler work; Register can be spammed by "Ensure" code paths.
    self:_RequestScheduleNext()
end
...
function UM:Kick(name)
    ...
    -- Perf: coalesce repeated Kick() calls during event storms.
    -- If we're already awake on the main scheduler OnUpdate, don't churn timers/scripts again.
    if not self._kickPending then
        self._kickPending = true
        if C_Timer and C_Timer.After then
            C_Timer.After(0, self._kickClearFn)
        else
            -- No timer API; clear immediately.
            self._kickPending = nil
        end
    end
    ...
    -- Ensure we recompute the next due time next frame (coalesced).
    self:_RequestScheduleNext()
end
...
function UM:SetEnabled(name, enabled)
    ...
    -- Perf: avoid doing any work if state doesn't change.
    if e.enabled == on then
        -- If the entry is enabled but somehow not in activeIndex, re-activate, otherwise no-op.
        if on and self.activeIndex and not self.activeIndex[name] then
            self:_Activate(name)
            self:_RequestScheduleNext()
        end
        return
